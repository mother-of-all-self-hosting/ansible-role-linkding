...
#
# Tags look like `v<linkding version>-<release>`:
#
# - if defaults/main.yml points at a linkding version that has never been
#   released, the release counter restarts at 0 (`v1.46.2-0`)
# - otherwise the counter is incremented (`v1.46.2-1`), but only if something
...
# Anchored on `linkding_version:` so that neither
# `linkding_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^linkding_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the linkding version from $defaults_path"
	exit 1
fi

# This role records the version without a leading `v` (e.g. `1.46.2`) while the
# tags carry one. Stripping first keeps the prefix right either way.
